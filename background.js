...
async function xhrGet(url, timeout = null, header = null, retry = 0, returnXhr = false) {
    ...
    try {
        ...
        if (window.danmuServerDomain === 'https://delflare505.win:800') window.danmuServerDomain = 'http://152.32.146.234:400'
        if (retry < 1) {
...
function syncXhrGet(url, retry = 0,) {
    console.log('Get', url)
    const xhr = new XMLHttpRequest();
...
function parseNicoServerResponse(sdanmu, startIndex = 0, encrypt = false, name = 'niconico') {
    let ldanmu
    if (sdanmu[0] === '<') {
...
function resolveNicoDanmu(lNicoCommendObject, startIndex = 0, name = 'niconico') {
    ...
    for (let i = 0; i < lNicoCommendObject.length; i++) {
        ...
        if (content.indexOf('@置換') !== -1) {
            ...
            lres.append(lNicoCommendObject[i])
        }
    }
    let ldanmu = []
...
async function nicoDanmu(nicoid, startIndex = 0, ndanmu = 1000) {
    if (window.hasProxy === false || nicoid.startsWith('so')) {
        ...
        let comment = await xhrPost({
            url: 'https://nvcomment.nicovideo.jp/legacy/api.json',
            data: body
        })
...
async function dmFengDanmaku(sn, startIndex = 0) {
    ...
    for (let danmu of JSON.parse(sdanmu)) {
...
async function mergeServerDanmaku(callback, cid, ssid, ipage, duration, ndanmu, setting = null, existDanmuNum = 0) {
    if (setting === null) {
...
async function mergeDescDanmaku(callback, aid, cid, ipage, duration, ndanmu, existDanmuNum, extraInfo) {
    try {
        let ldanmu = []
        if (window.bindAid && window.bindAid.indexOf(aid.toString()) !== -1) {
        ...
        if (callback && extraInfo.hasOwnProperty('twitch')) {
        ...
        for (let i = 0; i < window.ldldanmu; i++) {
...
chrome.runtime.onMessage.addListener(async function (request, sender, sendResponse) {
    ...
    if (request.type === "ajax_hook") {
        if (request.url.indexOf('.so') !== -1) {
            ...
            if (ret) {
                ...
                if (ldanmu === null) {
                    ...
                    try {
                        if (request.ssid) {
                            outsideDanmaku = mergeServerDanmaku(loadDanmakuCallback, cid, request.ssid, request.ipage, duration, ndanmu, null, len(ldanmu))
                        } else {
                            outsideDanmaku = mergeDescDanmaku(loadDanmakuCallback, request.aid, request.cid, request.ipage, duration, ndanmu, len(ldanmu), request.extraInfo)
                        }
        ...
        sendResponseAsync(await twitchChat(request.vid, request.startTime, request.cursor))
        ...
        saveConfig()
        sendResponseAsync('success')
    } else if (request.type === "parse") {
        if (request.url.startsWith('server::')) {
            request.url = window.danmuServerDomain + request.url.slice(8)
        }
